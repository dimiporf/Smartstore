...
{
    [LocalizedDisplay("Plugins.MyOrg.HelloWorld.Notification.")]
        ...
        IMapper<NotificationModel, Notification>
    {
        ...
        public NotificationModelMapper(SmartDbContext db, HelloWorldSettings settings)
        {
            _db = db;
            _settings = settings;
        }
        ...
        public async Task MapAsync(Notification from, NotificationModel to, dynamic parameters = null)
        {
            ...
            to.RemovalMessage = daysToRemoval <= 0 ?
                T("Plugins.MyOrg.HelloWorld.Notification.Grid.RemovalMessage.Today") :
                (daysToRemoval == 1 ?
                    T("Plugins.MyOrg.HelloWorld.Notification.Grid.RemovalMessage.Tomorrow") :
                    T("Plugins.MyOrg.HelloWorld.Notification.Grid.RemovalMessage.InXDays", daysToRemoval));
        }
